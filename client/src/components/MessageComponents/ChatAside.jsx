...
const ChatAside = ({ selectedUser, authUser }) => {
    ...
    const handleDisappearMessage = async (msg, flag) => {
        try {
            ...
            if (!flag) {
                console.log('disappearing message:', msg._id, flag);
                socket.emit('disappearMessage', msg);
            }
            ...
            <div className="">
                ...
                <div className="h-100 overflow-y-auto p-2 mt-4">
                    {messages.map((msg, index) => (
                        <div
                            key={index}
                            className={`p-2 my-1 ${msg.sender === authUser._id ? "text-right" : "text-left"
                                } flex gap-2`}
                        >
                            <span
                                className={`inline-block p-2 rounded-lg ${msg.sender === authUser.id
                            ...
                            <span
                                onClick={() => {
                                    sessionStorage.setItem("deletedMessage", JSON.stringify(msg));
                                    window.open(`/message/${msg._id})}`, '_blank');
                                    handleDisappearMessage(msg, user.id === msg.sender ? true : false, msg)
                                }}
